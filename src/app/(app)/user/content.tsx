...
import { Alert, Button, Container, Field, Heading, HStack, Input, NativeSelect, Separator, Spinner, Stack, Text } from '@chakra-ui/react';
...
/**
 * The consumer's own account page — edit name, gender, date of birth.
 *
 ...
 * account identity and isn't editable here; password has its own reset flow. If the
...
export function UserContent({ profile }: { profile: Keen.ConsumerProfile | null }) {
    ...
    const email = profile?.email ?? consumer.tokenPayload?.email;
    ...
    return (
        <Container maxW="lg" py={16}>
            <Stack gap={6}>
                <Button alignSelf="flex-start" variant="plain" onClick={() => router.push('/')}>
